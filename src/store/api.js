...
export default class Api {
  static fetchCheapFlights() {
    return axios.get('https://tokigames-challenge.herokuapp.com/api/flights/cheap').then(response => {
  ...
  static fetchBusinessFlights() {
    return axios.get('https://tokigames-challenge.herokuapp.com/api/flights/business').then(response => {
      if (response.status === 200) {
        return response.data;
      }
  ...
  static getAllFlights() {
    return axios.all([Api.fetchCheapFlights(), Api.fetchBusinessFlights()])
